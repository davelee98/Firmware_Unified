...
| 9.2 | `WifiConfig.server_host` | **gates nothing.** Retained in the 0x26 wire format. The field name misleads: `server_port` is the port *this device* binds, while `server_host` is a leftover from an abandoned "tag pushes to an upload server" model that the device-as-server design never used | n/a | **Do not delete it** — removing a field breaks the backward-compatibility rule (ARCHITECTURE.md). Document it as reserved-in-practice; a host may still write it. Worth a spec-comment correction so the next reader does not infer an outbound connection from the name. |

...
static void pollActivity() {
    ...
    // still registers. It wraps mod COMMAND_QUEUE_SIZE, but aliasing needs a whole queue of
    ...
    if (!activityPrimed) {
        activityPrimed = true;
               ...
               bleRxQueuePending() || od_txq_depth() > 0u) {
        lastActivityMs = od_hal_uptime_ms();
    }
...
void loop() {
    ...
    const bool workInFlight = bleRxQueuePending() || od_txq_depth() > 0u ||

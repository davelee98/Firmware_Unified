 ...
 *
 * These arrays are the fix: the ISR samples the pins itself and records the edge, so a press
 * shorter than the poll interval still survives to be published. s_irq_presses is a COUNT,
 * not a flag, so two quick taps between polls are not collapsed into one.
 */
...
/* Signalled by the ISR so the idle loop stops sleeping and publishes promptly instead of
 * waiting out the rest of its 1000 ms chunk. Consumed via opendisplay_button_wait(). */
 ...
 * MAX_BUTTONS register reads, cheap enough for an ISR.
 */
...
void opendisplay_button_process(void)
{
  for (uint8_t i = 0; i < s_button_count; i++) {
     ...
     * press that has already been released is invisible to the live read, and that is exactly
     * the case the old poll-only path lost.
     */

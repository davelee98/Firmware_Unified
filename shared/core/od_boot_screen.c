...
static void makeGray4PlaneRow(uint8_t *planeRow, const uint8_t* row2bpp,
                              int planePitch, uint16_t w, int bitSel) {
...
static const uint8_t kSchemeWhiteValue[] = {
    ...
    0xFF,  // 7: OD_COLOR_SCHEME_SEVEN_COLOR — currently rendered as a mono stub
    0x11,  // 8: OD_COLOR_SCHEME_BWGBRY_SPLIT — same packed white as BWGBRY
};
...
static bool bootGray4PanelUsesLutV2(uint16_t panelIc) {
    // Mirrors the authority and py-opendisplay's _GRAY4_CODES_BY_PANEL.
    return panelIc == 0x0028 || panelIc == 0x0048;
}

static void bootGray4FillSwatchCodes(uint16_t panelIc, bool direct2bpp, uint8_t out[4]) {
...
bool od_boot_screen_render(const struct od_config *cfg,
    ...
    const uint16_t h = cfg->displays[0].pixel_height;
    const uint8_t rotation = cfg->displays[0].rotation; // 0=0°,1=90°,2=180°,3=270°
    ...
    if (useZoneLayout && footerH > footerPadTop + 20) {
        switch (colorScheme) {
            ...
            case OD_COLOR_SCHEME_GRAY4:
                bootGray4FillSwatchCodes(cfg->displays[0].panel_ic_type, direct2bpp, swatchCode);
                numSwatches = 4;
                break;
    ...
    char last6[7];
    const uint32_t last3 = od_boot_app_device_id24() & 0xFFFFFFu;
    snprintf(last6, sizeof(last6), "%06lX", (unsigned long)last3);

    char url[OD_BOOT_URL_SIZE];
    if (!od_boot_url_build(cfg->displays[0].legacy_tag_type, last3, key, showKey,
                           cfg->manufacturer_data.manufacturer_id, url, sizeof(url))) return false;

    QRCode qr;
    const uint8_t qrVersion = 6;
    uint16_t qrBufSize = qrcode_getBufferSize(qrVersion);
    if (qrBufSize == 0 || qrBufSize > bufs->qr_len) return false;
    if (qrcode_initText(&qr, bufs->qr, qrVersion, ECC_MEDIUM, url) != 0) return false;
...
// IF NOT PRESENT, THEY WILL BE EMPTY STRINGS.
    ...
    if (cfg->data_extended_loaded) {
        snprintf(manufLine, sizeof(manufLine), "%s",
                 (const char *)cfg->data_extended.manufacturer_name);
        snprintf(modelLine, sizeof(modelLine), "%s",
                 (const char *)cfg->data_extended.model_name);
    }
#endif
    ...
    od_boot_app_firmware_version(&fwMajor, &fwMinor, &fwPatch);
    if (useZoneLayout) {
        snprintf(nameLine, sizeof(nameLine), "ID:   OD%s", last6);
        snprintf(fwLine, sizeof(fwLine), "FW:   OD ver %u.%u.%u",
                 (unsigned)fwMajor, (unsigned)fwMinor, (unsigned)fwPatch);
    } else {
    ...
    char k1[24], k2[24];
    if (useZoneLayout) {
        od_boot_format_key_line("KEY1:", key, 8, od_boot_key_is_zero(key), showKey,
                                k1, sizeof(k1));
        od_boot_format_key_line("KEY2:", key + 8, 8, od_boot_key_is_zero(key), showKey,
                                k2, sizeof(k2));
    } else {
        char keyHex[33];
        od_boot_format_key_display(key, showKey, keyHex);
        memcpy(k1, keyHex, 16);
        k1[16] = '\0';
    ...
    char tStr[8] = "";
    if (useZoneLayout) {
        float batV = od_boot_app_battery_volts();
        float chipT = od_boot_app_chip_temp_c();
        if (batV >= 0.0f) snprintf(vStr, sizeof(vStr), "%.2fV", (double)batV);
        else snprintf(vStr, sizeof(vStr), "--V");
        if (chipT > -900.0f) snprintf(tStr, sizeof(tStr), "%.1fC", (double)chipT);
        else snprintf(tStr, sizeof(tStr), "--C");
    }

    int middleScaleText = 1;
    int pad = 6;
    int fwKey1Gap = 0;
    int modulePx = 1;
    int qrPx = (int)qrModules;
    bool qrRight = false;
    int qrX = 0;
    int qrY = 0;
    int availW = (int)w_log;
    int textY = 0;
    ...
    if (useZoneLayout && vStr[0] != '\0') {
        ...
        if (useHighResLayout) {
            ...
            if (cfg->displays[0].partial_update_support) {
    ...
    if (useZoneLayout) {
        ...
        int logoScale = 1;
#if OD_BOOT_LOGO_SIZES >= 3
        if (BOOT_LOGO_H_S3 <= maxLogoH && BOOT_LOGO_W_S3 < maxLogoW) logoScale = 3;
        else
#endif
#if OD_BOOT_LOGO_SIZES >= 2
        if (BOOT_LOGO_H_S2 <= maxLogoH && BOOT_LOGO_W_S2 < maxLogoW) logoScale = 2;
#endif
#if OD_BOOT_LOGO_SIZES >= 3
        if (logoScale >= 3) {
    ...
    const bool gray4Split = (colorScheme == OD_COLOR_SCHEME_GRAY4) && !direct2bpp;
    const int planePitch = (w + 7) / 8;
    const int planePasses = (gray4Split || colorSwatchPlane1) ? 2 : 1;
    const uint8_t segments = od_boot_app_segments();
    const size_t rowRequired = (size_t)pitch + (gray4Split ? (size_t)planePitch : 0u);
    bool outputOk = true;
    if ((segments != 1u && segments != 2u) || (pitch % segments) != 0 ||
        (gray4Split && (planePitch % segments) != 0) || rowRequired > bufs->row_len) {
        return false;
    }
    if (od_boot_app_begin_frame(w, h, segments) != 0) return false;
    for (uint8_t segment = 0; segment < segments && outputOk; segment++) {
      for (int pass = 0; pass < planePasses && outputOk; pass++) {
        const int bitSel = pass;  // pass 0 -> LSB/PLANE_0, pass 1 -> MSB/PLANE_1
        const int targetPlane = (gray4Split || colorSwatchPlane1)
            ? (pass == 0 ? OD_BOOT_PLANE_PRIMARY : OD_BOOT_PLANE_SECOND)
            : (useBitplanes ? OD_BOOT_PLANE_PRIMARY : od_boot_app_default_plane(colorScheme));
        ...
        if (od_boot_app_begin_plane(targetPlane) != 0) {
            outputOk = false;
            break;
        }
        for (uint16_t y_native = 0; y_native < h && outputOk; y_native++) {
            ...
            for (uint16_t x_native = 0; x_native < w; x_native++) {
                ...
                bool black =
                    ...
                    || (useZoneLayout && logoW > 0 && bootLogoPixelBlack(lx, ly, logoX, logoY, logoBmp, logoW, logoH, logoStride, (int)w_log))
                    ;
                if (black) {
            ...
            if (gray4Split) {
                uint8_t *planeRow = row + pitch;
                makeGray4PlaneRow(planeRow, row, planePitch, w, bitSel);
                if (od_boot_app_write_row(y_native, segment,
                        planeRow + segment * (planePitch / segments),
                        (uint16_t)(planePitch / segments)) != 0) outputOk = false;
            } else {
                if (od_boot_app_write_row(y_native, segment,
                        row + segment * (pitch / segments),
                        (uint16_t)(pitch / segments)) != 0) outputOk = false;
            }
        }
        if (od_boot_app_end_plane(targetPlane) != 0) outputOk = false;
      }

      if (useBitplanes && !colorSwatchPlane1 && outputOk) {
            memset(row, 0x00, pitch);
            if (od_boot_app_begin_plane(OD_BOOT_PLANE_SECOND) != 0) outputOk = false;
            for (uint16_t y = 0; y < h && outputOk; y++) {
                if (od_boot_app_write_row(y, segment,
                        row + segment * (pitch / segments),
                        (uint16_t)(pitch / segments)) != 0) outputOk = false;
            }
            if (od_boot_app_end_plane(OD_BOOT_PLANE_SECOND) != 0) outputOk = false;
        }
    }
    if (od_boot_app_end_frame() != 0) outputOk = false;
    return outputOk;
}

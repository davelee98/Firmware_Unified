...
**Status:** SOFTWARE IMPLEMENTATION CANDIDATE 2026-08-17 on `codex/silabs-c13`; hardware Gate 2
remains UNRUN. Shared host tests (including the 2,048-byte/three-slot capability build) and the
BG22 headless build pass. The last measured image is 250,728 B flash and 32,284 B in the size
tool's data-plus-BSS summary (480 B main-RAM headroom). That summary includes the elastic linker
heap and therefore stays constant: the map moved `__HeapBase` from `0x200056a8` to `0x200052a8`
and grew `heap_size` from `0x2958` to `0x2d58`, a measured **1,024 B static-RAM reduction**.
The target-production Silabs corpus in § 6.2 now compiles the real BG22 command hooks against fake
drivers and passes all 11 applicable H2D checks (including all eight target-production vectors).
The production-source Silabs fault suite adds 258 assertions for config persist-before-queue and
failure isolation, CONFIG_CLEAR, byte-identical RETRY, the 22-chunk/two-slot producer, both
transport deadlines, write-before-subscribe, NFC 218/219, and every DIRECT END completion-barrier
outcome (zero-pending, API error, corrupt/overflow, deadline, and tag replacement). Remaining § 6.1
items are called out below; hardware Gate 2 is still incomplete, so do not call C13
hardware-verified or merge it as complete until § 7's board rows pass.

**REVISED SEVEN TIMES ON 2026-08-17.** The seventh pass makes the BLE event layer the sole owner of
post-disconnect advertising restart. The SDK callback forwards the close event but cannot restart
again; a structural gate pins exactly one private restart call in the close handler. Advertising
data, timing, and start API failures enter Gecko AppLoader instead of reaching the live
`app_assert` loop.

**REVISED SIX TIMES ON 2026-08-17.** The sixth pass separates boot API failures from valid degraded
MTU outcomes. Failure of either required boot configuration API enters Gecko AppLoader instead of
asserting or exposing a partial service. A successful lower stack MTU and an absent/lower peer MTU
exchange remain usable at the effective negotiated ceiling, starting from ATT MTU 23.

**REVISED FIVE TIMES ON 2026-08-17.** The fifth pass corrected the config-save ordering: persistence
must succeed before a success ACK is sealed and queued. The old session remains live long enough to
seal that ACK, then configuration reloads and the session is cleared. A persistence failure emits
no success ACK and leaves the previously stored configuration and its session intact.
...
returns `NOT_SUPPORTED` today), that `SL_PSA_KEY_USER_SLOT_COUNT` is 0 against a 4-slot pool shared with the
...
**The Silabs stack must also request ATT MTU 256 explicitly.** `SL_BGAPI_MAX_PAYLOAD_SIZE` bounds the
configured MTU to `payload_size - 7`, so set it to at least **263**. During the system-boot event,
before advertising, call `sl_bt_gatt_server_set_max_mtu(256, &selected)`. An API error means the
application cannot establish the stack contract and must enter Gecko AppLoader immediately. A
successful lower `selected` value is different: retain it as the degraded stack ceiling and keep
the application reachable. Every connection begins at ATT MTU 23 and adopts the exchange-complete
event's value up to that ceiling, so the radio HAL always respects effective `ATT_MTU - 3`; the
fleet maximum remains MTU 256 / value 253. If negotiation does not complete, that connection
continues safely at the default MTU 23; it does not block boot.
...
| **D2** | **Session never cleared after a config save.** Change the encryption key over an old session and the old session keeps working | `DIVERGENCE_MATRIX:207` (2.4) — `clear_session` sites `:118,431,625,677,1267`, none post-save | **Nordic's order is load-bearing:** persist successfully first; only then seal and queue the ACK under the OLD session; reload configuration; finally call `od_session_clear()`. A failed save queues no success ACK and must not reload or clear the still-valid old session. **NOT `od_core_reset()`**: it calls `od_txq_reset()` (`od_core.c:20`), which would drop an ACK already queued, or invalidate the handler's reservation if called earlier. `od_core_reset()` is teardown/failure reset for disconnect and event-hold expiry, never config save. Template: `targets/nordic-zephyr/src/od_cmd_config.c:218-228` |
...
**THE ENABLING MUST SURVIVE IGNORED AUTOGENERATED FILES.** `autogen/` is gitignored and
`cmake_gcc/opendisplay-bg22.cmake` is hand-maintained (workspace `CLAUDE.md`), so a local edit to
`sli_psa_config_autogen.h` is not an implementation: a clean checkout loses it. C13 therefore
selects a tracked PSA override from `config/od_psa_config_autogen.h`, and records
`psa_crypto_ccm`, `bluetooth_feature_resource_report`, and the one-user-slot setting in the tracked
`.slcp` for a future safe regeneration. **An earlier draft also said to add the AEAD driver source;
that was false:**
`sli_cryptoacc_transparent_driver_aead.c` is already in
`cmake_gcc/opendisplay-bg22.cmake:205`. A later build can still silently lose the CMake override or
the `.slcp` component selections. C13.3 therefore ratchets both tracked inputs in `tools/check.sh`;
the failure mode otherwise is "encryption stopped working after a clean checkout or
regeneration", discovered on hardware.
...
pre-C13 map reported `heap_size = 0x2958` = **10,584 B**. The implementation map reports
`__HeapBase = 0x200052a8` and `heap_size = 0x2d58` = **11,608 B**, versus the pre-C13
`__HeapBase = 0x200056a8`. The elastic heap therefore grew by **1,024 B**, directly measuring a
1,024 B reduction in non-heap static allocation. `arm-none-eabi-size` still reports 32,284 B because
it counts the linker-created heap in data plus BSS; that aggregate is not the C13 acceptance metric.
...
| **C13.0b** | `OD_CONFIG_MAX_SIZE` becomes an `#ifndef` target macro with the once-only 400-byte **policy** floor in `od_config_asm.h`; BG22 sets 2048 and ESP32/Nordic assert 4096. Retain `OD_TX_FRAME_MAX=256`. Raise the dynamic characteristic's maximum value length 244 → 253 (so the 245..253 oversize NACK is reachable) and decide the long-write staging width (§ 2.3). Configure `SL_BGAPI_MAX_PAYLOAD_SIZE >= 263`; request maximum ATT MTU 256 during system boot before advertising, enter Gecko AppLoader on API failure, and use a successful lower selection plus each connection's negotiated MTU as degraded ceilings. Add `shared/core/od_caps.h` and the one-source routing/budget metadata (§ 2.3); BG22 sets `OD_CAP_PIPE=0`, `OD_CAP_PARTIAL=0` and `OD_CAP_RXQ=0`. Amend CLAUDE.md decision 12 and `DIVERGENCE_MATRIX` 2.7; add D8 to the matrix; record 0x04 as § 2.1's target-specific unsupported-PIPE marker | **Untouched-device capture or conscious skip is already recorded before this commit.** Host: oversize declared total REJECTED at the start frame with nothing stored at both caps; 253-byte BLE reply queued and 254 refused while the TX slot remains 256; metadata row/capability tests green; ESP32/Nordic builds unchanged; boot log records selected MTU 256; host transport tests cover successful lower selection, MTU-23-until-exchange fallback, and exchanged-MTU enforcement; hardware fault injection confirms an API error enters AppLoader. **This commit changes the reachable wire surface:** a 245..253-byte write now reaches dispatch and gets its oversize NACK instead of transport refusal |
| **C13.1** | Add the Silabs build to `tools/check.sh` as a fourth target gate (`DO_SILABS`, folded into `--targets`), under the same skip-is-not-a-pass rule | `--targets` reports 16/0/0 with the toolchain present, and a **skip** without it |
...
| **C13.6** | Split commands into `od_cmd_{device,config,direct,nfc}.c`; define all 21 hooks **plus `od_core_frame_done()`, `od_cmd_allow_unauthenticated()` and `od_cmd_mutates_config()`** (§ 4.4); adopt `od_dispatch_frame()`. Use `od_core_reset()` for **disconnect and § 2.2 event-hold expiry only**, never config save. Every successful config-save path uses the exact D2 order from § 3: persist → seal/queue ACK under the old session → reload → `od_session_clear()`; failure emits no success ACK, reload or clear. Add `od_config_read` **with the § 2.4 producer deadline** and the `sl_bt_can_process_event()` override. Delete the opcode switch (`:1089-1195`) and byte-inferred sealing. Closes **D1, D2, D6, D7** and settles 0x0080 | A missing hook is a link error — that is the enforcement; corpus gains `dispatch_corpus_silabs` (§ 6.2), incl. CONFIG_CLEAR success/persistence vectors and BG22's unauthenticated/mutates-config answers. Inject persistence success and failure: success proves persist precedes ACK and that reload/clear follow it; failure proves no success ACK, reload or clear occurs and the prior config/session remain usable. Also prove DEFERRED logging and producer-deadline expiry; **hardware Gate 2 full** |
| **C13.7** | Split BG22 display END into prepare/refresh, queue the ACK, drain it to stack acceptance in-handler, then use `bluetooth_feature_resource_report` and `sl_bt_resource_get_connection_tx_status()` against the same deadline to wait for zero packets pending on the exact connection instance before refresh. Failure to enable required TX reporting at boot enters Gecko AppLoader. On runtime timeout/report error/tag change, do not refresh: log, reset transport state and close that instance. Keep the 2-unit reservation across refresh (**D5**, § 4.5). Also cap NFC at 218 and make builders fail rather than truncate (**D8**) | Host fake proves acceptance alone cannot satisfy the barrier and covers zero-pending, timeout, corrupt/overflow flags and tag replacement. Hardware/sniffer: boot enable failure enters AppLoader; END ACK is observed on air before refresh; resource report corroborates zero pending; post-refresh `0x73`/`0x74` arrives from held reservation; runtime failure injection does not refresh. 218 arrives whole; 219 errors rather than truncates. Record resource-report `.bss`/`.text` cost |
| **C13.8** | Extend the C11 ownership ratchets to this target; update CLAUDE.md status, `DIVERGENCE_MATRIX` resolutions, `docs/TEST_OWNERSHIP.md` | Clean-tree `--targets` at 16/0/0; every closed matrix row edited to say so |
...
- **Config-save ordering:** instrument the storage, reply, reload and session seams. On success the
  only accepted sequence is persist → seal/queue success ACK under the old session → reload → clear.
  Injected persistence failure emits a failure response only, never a success ACK, and leaves the
  previous configuration and session usable.
...
**Implementation status (2026-08-17):** `silabs_fault_test.c` closes the target-policy portions of
config-save ordering, egress RETRY identity, the exact 22-chunk producer, write-before-subscribe,
both deadlines, D5 completion, and the command-level NFC 218/219 boundary by compiling the real
Silabs command/transport/radio translation units. Still open in this section are a captured
pre-fix failure run, the retired-parser differential, a Silabs-specific PSA lifecycle/differential
arm, explicit DEFERRED log capture, and a direct unit test of the production NDEF builder's
too-small-`out_max` return (the command-level fake verifies that 219 cannot become a success frame,
but does not compile the static builder in `opendisplay_ble.c`).
...
summary after C13.1: **16 passed, 0 failed, 0 skipped**.
...
| Config single write, chunked write, read-back, reboot | persistence succeeds before the success ACK is queued; ACK precedes reload and session clear; bytes survive reboot | C13.2 plus C13.6 ordering / not the cap |
...
- **Two config buffers must never coexist.** If `opendisplay_config_buf()` and `s_cfg_chunk` cannot
  ...
  unestimated work.
- **The final static delta must remain negative.** The implementation map satisfies this condition:
  `__HeapBase` moved down by 1,024 B and the elastic `heap_size` grew by 1,024 B. The result is better
  than the −533 B prediction; the size tool's unchanged data-plus-BSS aggregate includes that heap
  and is not evidence of a zero delta. If a later link loses this negative non-heap delta, stop and
  revise the architecture; static RAM reduction is the premise that makes this migration viable.
...
- **Stop if the AEAD enabling cannot be ratcheted.** It lives in a tracked PSA override and the
  `.slcp` component selection (§ 4.2); without checks for both, a clean checkout or later
  regeneration can disable all session encryption silently and the next evidence is a device that
...
- every config-save path persists before queueing success, queues that ACK under the old session,
  then reloads and clears; an injected persistence failure cannot emit success or disturb the prior
...
- a 2,000 ms target-level maximum event-hold/TX-retry lifetime retires a stuck RETRY head by logging
  and resetting command transport state; CONFIG_READ has its own 1,500 ms producer deadline and a
  compile-time ordering check, so producer cancellation alone is not claimed as the outer bound;
...
- `tools/check.sh --targets` reports 16 passed, 0 failed, 0 skipped from a clean tree;
